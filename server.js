...
function normalizeLobbyRecord(b = {}, updatedAt = Date.now()) {
    return {
        code: b.code,
        name: b.name || 'Lobby',
        hostName: b.hostName || 'Host',
        players: Math.max(1, Math.min(16, Number(b.players) || 1)),
        map: b.map || 'Unknown',
        mode: b.mode || 'Classic',
        ...
        allowLateJoin: b.allowLateJoin !== false,
        updatedAt
    };
}
...
const server = http.createServer(async (req, res) => {
    ...
    if (urlPath === '/api/lobbies' && req.method === 'POST') {
        ...
        lobbies.set(b.code, normalizeLobbyRecord(b));
