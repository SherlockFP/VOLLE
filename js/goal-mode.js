 ...
 */
export function checkGoalEntry(position, zones) {
    const none = { scored: false, scoringTeam: null, concededTeam: null, zone: null };
...
export function applyGoalScore(state, scoringTeam, meta = {}) {
    ...
    const next = {
        ...state,
        redScore: state.redScore + (scoringTeam === 'red' ? 1 : 0),
        blueScore: state.blueScore + (scoringTeam === 'blue' ? 1 : 0),
...
export function resolveGoalRushTick(state, ballPosition, zones, meta = {}) {
    if (!state) return { state, entry: { scored: false, scoringTeam: null, concededTeam: null, zone: null } };
    const entry = checkGoalEntry(ballPosition, zones);
    if (!entry.scored) return { state, entry };
    const ownGoal = meta.shooterTeam ? meta.shooterTeam === entry.concededTeam : false;
    const nextState = applyGoalScore(state, entry.scoringTeam, { ...meta, ownGoal });
