...
export class Player {
    ...
    setHandVisible(on) {
        ...
        [this.armMesh, this.handMesh, this.gloveMesh].forEach(mesh => {
            if (mesh) mesh.visible = false;
        });
        if (this.knifeGroup) this.knifeGroup.visible = visible;
    }

    _applyKnifeIdlePose() {
        if (this.knifeGroup?.userData.weaponType !== 'knife') return;
        this.knifeGroup.position.set(0.16, 0.04, -0.46);
        this.knifeGroup.rotation.set(-0.34, 0.36, -0.72);
    }
    ...
    update(dt) {
        ...
        // Swing anim
            ...
            if (this.knifeGroup?.userData.weaponType === 'knife') {
                this.knifeGroup.rotation.z = -0.72 + swing * 1.25;
                this.knifeGroup.rotation.x = -0.34 - swing * 0.4;
            }
        } else {
