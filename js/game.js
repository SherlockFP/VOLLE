...
addRemotePlayer(playerId, name = 'Player', team, avatarDataUrl = null, peerId = playerId) {
    ...
    removeRemotePlayer(playerId) {
        ...
        // Cosmetics that rode a rig socket live outside cosmeticsRoot — dispose them too.
    ...
    _createRemotePlayer(peerId, name, team, avatarDataUrl) {
        ...
        knifeGroup.position.set(0, -0.02, -0.1);
        knifeGroup.rotation.set(-0.3, 0, -0.15);
        rig.sockets.handR.add(knifeGroup);
        ...
        const p = {
            peerId, name, team, group, rig, animator,
            playerId: peerId,
            knifeGroup,
            targetOutline,
            ...
            setKnifeStyle(style = KNIVES.training) {
                ...
                nextKnife.position.copy(this.knifeGroup ? this.knifeGroup.position : new THREE.Vector3(0, -0.02, -0.1));
                nextKnife.rotation.set(-0.3, 0, -0.15);
                disposeObject3D(this.knifeGroup);
                this.knifeGroup = nextKnife;
                this.rig.sockets.handR.add(nextKnife);
            },
...
spawnPowerUp() {
    ...
    // (speed/verticalSpeed/grounded) are derived purely from position deltas — the
    // plan deliberately keeps animation state off the network (0 bandwidth cost).
    // ponytail: reused facts object + plain-number prev position, 0 alloc/frame.
    _stepRemoteAnimator(p, dt) {
        if (!p.animator) return;
        const prevX = p._animX ?? p.position.x;
        const prevY = p._animY ?? p.position.y;
        const prevZ = p._animZ ?? p.position.z;
        p._animX = p.position.x;
        p._animY = p.position.y;
        p._animZ = p.position.z;
        const invDt = dt > 1e-4 ? 1 / dt : 0;
        const speed = Math.hypot(p.position.x - prevX, p.position.z - prevZ) * invDt;
        const verticalSpeed = (p.position.y - prevY) * invDt;
        const grounded = Math.abs(verticalSpeed) < 1.2;
        if (p._animFalling && grounded) p.animator.play('land');
        p._animFalling = !grounded && verticalSpeed < -0.4;
        const facts = p._animFacts;
        facts.speed = speed;
        facts.grounded = grounded;
...
// Crops the face region out of an uploaded Minecraft-style skin so the rig's
// head texture shows just the face, not the whole skin sheet.
...
// Avatar pixel color extraction — reads skin data and averages per-part regions
// (body/arms/legs), then hands the hex colors to rig.setPartColors(). Regions with
// no visible (alpha>128) pixels fall back to fallbackColor (the player's team color).
function _applyAvatarColors(img, p, fallbackColor) {
    ...
    const colors = size === 64 ? {
        body: avgHex(20, 20, 8, 12) ?? fallbackColor,
        arms: avgHex(44, 20, 4, 12) ?? fallbackColor,
        legs: avgHex(4, 20, 8, 12) ?? fallbackColor
    } : {
        body: avgHex(0, 4, 16, 4) ?? fallbackColor,
        arms: avgHex(0, 8, 16, 4) ?? fallbackColor,
        legs: avgHex(0, 12, 16, 4) ?? fallbackColor
    };
    p.rig.setPartColors(colors);
}
