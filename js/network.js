...
const encodeShortText = value => new TextEncoder().encode(String(value ?? '')).slice(0, 255);

export function localJoinCosmetics(store = globalThis.window?.__store) {
    const skinId = store?.get?.('equippedAvatarSkin') || 'default';
    return {
        avatar: store?.get?.('customAvatar')?.dataURL || createAvatarDataURL(skinId) || '',
        ballSkinId: store?.get?.('equippedBall') || 'classic',
        ballTrailId: store?.get?.('cosmeticLoadout')?.ballTrail || 'none'
    };
}
...
export function hasSimulationAuthority(network) {
    return !network?.connected || network.isHost === true;
}

export function normalizeCountdownState(value = {}) {
    const phase = ['pre', 'final'].includes(value.phase) ? value.phase : 'idle';
    return {
        phase,
        remaining: phase === 'idle' ? 0 : Math.max(0, Number(value.remaining) || 0),
        duration: Math.max(1, Number(value.duration) || 1)
    };
}
...
export class Network {
    ...
    async _joinGame(roomCode, playerName, password = '') {
        ...
        return new Promise((resolve, reject) => {
            conn.on('open', () => {
                ...
                const cosmetics = localJoinCosmetics();
                conn.send({ type: 'join', name: playerName, password, ...cosmetics, playerId: this.playerId });
                resolve();
            });
    ...
    _reconnectOnce() {
        ...
        conn.on('open', () => {
            ...
            this.setupDataHandlers(conn);
            const cosmetics = localJoinCosmetics();
            conn.send({ type: 'join', name: this.playerName, password: this.joinPassword, ...cosmetics, playerId: this.playerId });
    ...
    _handleJoinConn(conn) {
        ...
        const resuming = Boolean(expectedToken && expectedToken === resumeToken);
        if (this.game?.shouldQueueLateJoin?.() && !this.allowLateJoin && !resuming) {
            conn.send({ type: 'kick', name, reason: 'late_join_disabled' });
            setTimeout(() => conn.close(), 200);
            return;
        }
        ...
        conn._sendWelcome = () => {
            const snapshot = this.game.snapshotState?.() || {};
            conn.send({
                type: 'welcome',
    ...
    _decodeBinary(data) {
        try {
            ...
            if (t === BIN.BALL) return dv.byteLength >= 32 ? this._decodeBallState(dv) : null;
            if (t === BIN.POS) return dv.byteLength >= 30 ? this._decodePosition(dv) : null;
        } catch (_) {
            return null;
        }
        return null;
    ...
    _decodeBallState(dv) {
        ...
        msg.active = !!(flags & 1);
        let off = 32;
        if (flags & 2) { const sc = dv.getUint8(off); off += 1; msg.state = { 0: 'idle', 1: 'rally', 2: 'hold', 3: 'warmup', 4: 'other', 5: 'homing', 6: 'falling', 7: 'orbiting', 8: 'held' }[sc] || 'rally'; }
        ...
        if (flags & 16) { const len = dv.getUint8(off); off += 1; msg.targetId = len ? new TextDecoder().decode(new Uint8Array(dv.buffer, dv.byteOffset + off, len)) : null; off += len; }
        if (flags & 32) { const len = dv.getUint8(off); off += 1; msg.skinId = new TextDecoder().decode(new Uint8Array(dv.buffer, dv.byteOffset + off, len)); off += len; }
        if (flags & 64) { const len = dv.getUint8(off); off += 1; msg.trailId = new TextDecoder().decode(new Uint8Array(dv.buffer, dv.byteOffset + off, len)); off += len; }
        return msg;
    }
    ...
    encodeBallState(b) {
        ...
        const hasTargetId = Object.prototype.hasOwnProperty.call(b, 'targetId');
        const hasAffix = !!b.affix;
        const hasSkin = typeof b.skinId === 'string';
        const hasTrail = typeof b.trailId === 'string';
        let size = 32;
        let stateCode = 1, targetBytes = null, targetIdBytes = null, affixBytes = null, skinBytes = null, trailBytes = null;
        if (hasState) { size += 1; stateCode = { idle: 0, hold: 2, warmup: 3, rally: 1, other: 4, homing: 5, falling: 6, orbiting: 7, held: 8 }[b.state] ?? 4; }
        if (hasTarget) { targetBytes = encodeShortText(b.targetName); size += 1 + targetBytes.length; }
        if (hasTargetId) { targetIdBytes = encodeShortText(b.targetId); size += 1 + targetIdBytes.length; }
        if (hasAffix) { affixBytes = encodeShortText(b.affix.id || b.affix.name); size += 1 + affixBytes.length + 4; }
        if (hasSkin) { skinBytes = encodeShortText(b.skinId); size += 1 + skinBytes.length; }
        if (hasTrail) { trailBytes = encodeShortText(b.trailId); size += 1 + trailBytes.length; }
        ...
        if (hasTargetId) { flags |= 16; dv.setUint8(off, targetIdBytes.length); off += 1; u8.set(targetIdBytes, off); off += targetIdBytes.length; }
        if (hasSkin) { flags |= 32; dv.setUint8(off, skinBytes.length); off += 1; u8.set(skinBytes, off); off += skinBytes.length; }
        if (hasTrail) { flags |= 64; dv.setUint8(off, trailBytes.length); off += 1; u8.set(trailBytes, off); off += trailBytes.length; }
    ...
    _validateMsg(data) {
        ...
        switch (data.type) {
            case 'join':
                return typeof data.name === 'string' && data.name.length > 0 && data.name.length <= 32
                    && (!data.avatar || (typeof data.avatar === 'string' && data.avatar.length <= 262144))
                    && (!data.ballSkinId || (typeof data.ballSkinId === 'string' && data.ballSkinId.length <= 32))
                    && (!data.ballTrailId || (typeof data.ballTrailId === 'string' && data.ballTrailId.length <= 32));
            case 'kick':
    ...
    handleMessage(data, peerId) {
        ...
        switch (data.type) {
            case 'join':
                if (!this.isHost) break;
                {
                    ...
                    if (this.onPlayerJoin) this.onPlayerJoin(data.name, playerId, data.avatar, peerId, {
                        ballSkinId: data.ballSkinId,
                        ballTrailId: data.ballTrailId
                    });
                    conn._sendWelcome?.();
                }
                break;
            ...
            case 'socialChat':
                ...
                break;
            case 'gameState':
                if (this.connected && !this.isHost && peerId === this.hostConn?.peer) {
    ...
    broadcastBallState(ball) {
        ...
        const targetId = target === this.game?.player
            ? this.playerId
            : (target?.playerId || target?.peerId || null);
        this._ballSeq = ((this._ballSeq || 0) + 1) & 0xffff;
