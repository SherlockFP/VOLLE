...
test('375px lobby gives the body one vertical scroll lane and keeps content columns non-shrinking', async () => {
    ...
    assert.equal(laterBodyRules.length, 1, 'no later mobile lobby rule may override the top-aligned scroll owner');
...
test('375px lobby topbar wraps into bounded, reachable rows without UUID overflow', async () => {
    ...
    assert.match(mobile, /#lobby-screen \.cs-topbar\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\) auto auto;[\s\S]*?"room room leave"[\s\S]*?"name name name"[\s\S]*?"info region network"[\s\S]*?max-height:\s*150px;[\s\S]*?overflow:\s*hidden;/);
    ...
    assert.match(mobile, /#lobby-screen \.lobby-region\s*\{[\s\S]*?grid-area:\s*region;/);
