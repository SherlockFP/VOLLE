...
test('bot respawn restores its render and combat state', async () => {
    ...
    assert.match(source, /tryDeflect\(ball, dt = 0\.016\) \{\s*if \(this\.observeDefenseIntent\(ball\) !== 'deflect'\) return false;/);
    assert.match(source, /this\._resetDefenseIntent\(\);\s*this\.alive = true;/);
