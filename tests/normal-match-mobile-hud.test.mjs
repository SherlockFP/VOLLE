...
async function hudLanesBlock() {
    ...
    return { css, block: css.slice(start), start };
}
...
test('375px and exact 720px normal matches reserve the scoped adjacent HUD lanes', async () => {
    const { block } = await hudLanesBlock();
    ...
    assert.match(block, /body:not\(\.practice-lab-active\) \.network-diagnostics\s*\{[\s\S]*?right:\s*12px;[\s\S]*?bottom:\s*156px;[\s\S]*?width:\s*95px;[\s\S]*?justify-content:\s*center;[\s\S]*?font-size:\s*0\.68rem;/);
    assert.match(block, /body:not\(\.practice-lab-active\) \.network-diagnostics > span\s*\{\s*display:\s*none;/);
});
...
test('normal-match controls wrap inside the 375px safe width while desktop stacks speed and diagnostics above the minimap', async () => {
    ...
    assert.match(block, /body:not\(\.practice-lab-active\) \.network-diagnostics\s*\{[\s\S]*?right:\s*16px;[\s\S]*?bottom:\s*196px;[\s\S]*?width:\s*160px;[\s\S]*?justify-content:\s*center;/);
});
