...
test('solo rewards are base-loss only, capped at three per UTC day, and restart replays do not consume cap', t => {
    ...
    const capped=id('solo4'); assert.equal(f.authority.start(f.a,{matchId:capped,mode:'solo'}).httpStatus,200); f.advance(100); assert.equal(f.authority.complete(f.a,{matchId:capped,mode:'solo'}).completion.soloCapped,true);
});
