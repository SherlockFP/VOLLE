...
test('guard is explicitly scoped to the first offline opposing bot without automatic FTUE hints', async () => {
    ...
    assert.match(game, /if \(!this\._isDeflectFacingBall\(aimDir, pos, this\.ball\.position\)\) \{[\s\S]*?this\.player\.attacking = false;[\s\S]*?this\.ui\.showMessage\?\.\(t\('match\.ballBehind'\), 900\);[\s\S]*?this\.audio\.playCue\?\.\('deflect-reject'\);/);
